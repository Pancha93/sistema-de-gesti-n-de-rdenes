...
import { ActualizarConfiguracionApiComponent } from '../../configuracionapi/actualizar-configuracionapi/actualizar-configuracionapi.component';
import { CrearConfiguracionApiComponent } from '../../configuracionapi/crear-configuracionapi/crear-configuracionapi.component';
...
@Component({
  selector: 'app-leer-configuracionapi',
  templateUrl: './leer-configuracionapi.component.html',
  standalone: true,
  imports: [
    FormsModule,
    CommonModule,
  ...
  styleUrls: ['./leer-configuracionapi.component.scss']
})
export class LeerConfiguracionApiComponent implements OnInit {
  // Columnas que se mostrarán en la tabla
  displayedColumns: string[] = ['id', 'nombreApi', 'urlBase', 'endpointLogin', 'campoUsuario', 'usuario', 'password', 'activo', 'descripcion', 'entidadAsociada', 'metodosSoportados', 'camposInterfaz', 'acciones'];
   ...
   * @param configuracionapiService Servicio para gestionar la entidad
  ...
  loadData(): void {
    this.configuracionapiService.findAll().subscribe({
      next: data => {
        this.dataSource.data = data.map(item => Object.assign(new ConfiguracionApiComponent(this.configuracionapiService), item));
   ...
   * @param {any} configuracionapi - El registro a editar
   * @description Abre un diálogo modal para editar el registro seleccionado
   */
  onEdit(configuracionapi: any): void {
    const screenWidth = window.innerWidth;
    const minWidth = screenWidth < 600 ? '90vw' : '800px';
    const dialogRef = this.dialog.open(ActualizarConfiguracionApiComponent, {
      minWidth: minWidth,
   ...
   * @param {number} configuracionapiId - ID del registro a eliminar
  ...
  onDelete(configuracionapiId: number) {
    ...
    const shouldProceed = this.preDelete(configuracionapiId);
    if (!shouldProceed) {
      return;
    }
    try {
      ...
      this.configuracionapiService.deleteById(configuracionapiId).subscribe(
        () => {
          ...
          this.postDelete(configuracionapiId);
        },
        error => {
  ...
  exportToExcel(): void {
    try {
      ...
      XLSX.writeFile(wb, 'configuracionapi_datos.xlsx');
